...
const SignUpPopup: FC<SignUpPopupProps> = ({ isOpen, onClose }) => {
  const router = useRouter();
  const [emailAuthClicked, setEmailAuthClicked] = useState(false);
  const [showPassword, setShowPassword] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const { signInWithGoogle, SignUpWithEmailAndPassword } =
  ...
  const submit: SubmitHandler<{ email: string; password: string }> = async (
    data: any
  ) => {
    try {
      setIsLoading(true);
      const signUpRes = await SignUpWithEmailAndPassword(
        data.email,
        data.password
      );
